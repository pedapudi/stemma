...
pub fn enqueue_missing_interpretations(db: &StemmaDb) -> Result<usize> {
    ...
    // 98 useful documents).
    type Cand = (String, String, i64, String, Option<i64>, String, String, bool);
    let candidates: Vec<Cand> = {
        let mut stmt = conn.prepare(
            "SELECT t.src_table, t.src_column, t.rep, t.value,
                    q.id, coalesce(q.status, ''), coalesce(q.content_hash, ''),
    ...
    let tx = conn.unchecked_transaction()?;
    {
        ...
        for (table, column, rep, value, qid, status, stored_hash, covered) in candidates {
