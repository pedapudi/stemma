...
fn background_task(name: &str, user_db: &std::path::Path, embed: Option<(String, String)>) {
    ...
    let embedder = embed
        .as_ref()
        .map(|(ep, model)| stemma_embed::OpenAiEmbedder::new(ep, model));
...
async fn main() -> anyhow::Result<()> {
    ...
    let embed = match (&args.embed_endpoint, &args.embed_model) {
        (Some(ep), Some(model)) => Some((ep.clone(), model.clone())),
        _ => None,
    };
    for (name, user_db) in &args.dbs {
