...
Status: **built** — `crates/stemma-eval` implements derivation
(`dataset`), the ablation runner (`run`), paired statistics, grading
(`grade` / `accept`) and the self-contained HTML report
(`eval/report/`). Honest deviations from the design as agreed, found in
building and running it:

This status applies to record-resolution evaluation. Query-level outcome and
clarification behavior exists in the runtime, as does the first validated
parser slice. The harness does not yet measure supported alternatives,
clarification dialogue, or parser acceptance. Their protocol is
...
- **Backend costs are metered at the seams.** LM round-trip time and
  ...
  `Embedder`/`LmBackend` traits inside the harness — no instrumentation
  was added to `stemma-resolve`. Per-query dense probe counts are
  approximated from the trace (spans carrying a dense channel score);
  total embedded texts are exact.
- **Containment (grading rule 2)** is checked between consecutive
  ablations within a run — that is where a mechanism's marginal movement
  lives — not against the baseline.
- **Layer 2** is currently run notes (index size, KG node/edge counts),
...
- The first run surfaced the coincidence rate the design predicted:
  ...
  the failure class denotation verification and two-strictness scoring
  exist to expose.

## Context

Every scoring constant in the pipeline today was set by reasoning and
spot-checks: the dense score mapping (`[0.30, 0.60] → [0, 0.78]`), the
...
- **Score-conditioned accuracy**: the observed gold-link rate in each of ten
  fused-score buckets. The plot reveals whether score ordering behaves
  differently across mechanisms and corpora. It describes the evaluated
  population. An unrepresentative gold set cannot turn the plot into a
  deployment probability or license a universal threshold.
...
Each run produces one self-contained HTML report. It contains the matrix,
deltas with confidence intervals, score-conditioned accuracy, and named
failures. The report uses the console's themes, type controls, and data
typography. A JSON artifact carries the same machine-readable results and
...
- **BIRD dev slice**: 5–8 databases spanning clean and dirty schemas,
  ...
  column codes), `debit_card_specializing` (abbreviation-heavy) — and
  three clean — `superhero`, `student_club`, `formula_1` (normalized,
  readable, join-rich). Denotation verification kept 545 of 777
...
- **Legal synthetic**: 175 questions: 50 anchor, 50 paraphrase, 50
  cross-record, and 25 absent. Questions were reverse-generated against
  sampled records, mechanically verified, and reviewed before freezing. The
  legal corpus has no join tables, so `mini-join-v1.jsonl` supplies 44 join
  records over 11 distinct cases. Both sets are versioned, and regeneration
  requires review.
...
### What the gold cases support

The checked-in SQL cases and synthetic questions are small and
mechanism-focused. They support regression checks, adversarial counterexamples,
ablation comparisons on the same cases, and inspection of named failures. They
do not estimate deployment accuracy because their sampling process does not
represent future use.

Reports keep controlled fixtures, gold SQL cases, and reviewed usage cases as
separate populations. A future representative usage sample may support
reliability estimation for that declared population. Distribution shift can
invalidate post-hoc calibration [Ovadia 2019], so a reliability estimate remains
scoped to its sampling process and database revision.

## Outside the resolution matrix

Execution accuracy is outside the record-resolution matrix because it also
measures query construction. Parser evaluation belongs in a separate layer
with gold-trace and predicted-trace runs. Cross-encoder reranking and mention
expansion enter the matrix as new ablation columns when implemented.

The separate
[`ambit_retrieval_study.py`](../../tools/ambit_retrieval_study.py) harness
compares bounded candidate-selection policies over one exported candidate pool.
It is implemented, but it does not replace the resolution matrix: it consumes
precomputed graph and geometry diagnostics, and it cannot establish deployment
accuracy from an unrepresentative input set. Its role is controlled mechanism
comparison under a shared candidate limit.

The implemented
[`dense_shadow_compare.py`](../../tools/dense_shadow_compare.py) tool serves a
different gate. It compares approximate and exact dense retrieval over paired
query vectors, using exact retrieval as an unlabeled fidelity oracle. Candidate
recall, leading-candidate agreement, missed ambiguity, false wide margins,
guarded outcome changes, and latency measure whether an accelerator preserves
the current dense channel. Runtime generation of the paired export remains
designed.
...
- [Ovadia 2019] Yaniv Ovadia et al. "Can You Trust Your Model's Uncertainty?
