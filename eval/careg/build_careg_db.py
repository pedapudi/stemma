#!/usr/bin/env python3
"""Builds the California Code of Regulations user database for stemma.
...
eval/careg/data/src/. Output: a stock SQLite database (eval/careg/data/careg.db)
...
(careg-*.parquet) can be joined in later without re-embedding.

Requires pyarrow. Usage:
    python3 eval/careg/build_careg_db.py [--src DIR] [--out FILE]
"""
...
def main():
    ...
    ap.add_argument("--out", default=os.path.join(here, "data", "careg.db"))
    args = ap.parse_args()
    ...
    for f in files:
        for batch in pq.ParquetFile(f).iter_batches(batch_size=2000):
            ...
            conn.executemany(
                "INSERT INTO regulations (uuid, text, license, category) VALUES (?, ?, ?, ?)",
                [
    ...
    avg_len = conn.execute("SELECT avg(length(text)) FROM regulations").fetchone()[0]
    conn.close()
    size_mb = os.path.getsize(args.out) / 1e6
    print(f"{args.out}: {n} regulations, avg text {avg_len:.0f} chars, {size_mb:.1f} MB")
